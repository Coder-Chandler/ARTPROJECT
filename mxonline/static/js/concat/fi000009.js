    ...
    $.fn.autocomplete = function(opts) {
      ...
      {@each _ as it,index}\
       <li class="clearfix {@if index <= 0}selected{@/if}" data-domain="${it.domain}" data-id="${it.uid}" data-name="${it.uname}">\
                                <div class="head clearfix">\
                                    <a href="http://js.artand.cn/${it.domain}" class="btn imgbtn">\
         <img src="${it.head}"></a>\
                                </div>\
                                <div class="writing">\
          ...
          {@if it.is_verfiy > 0}\
           {@if (it.role ==1)}\
            <span class="approve-btn-a btn approve" href="http://js.artand.cn/verify"></span> \
           {@/if}\
           {@if (it.role ==4)}\
           <span class="approve-btn-c btn approve" href="http://js.artand.cn/verify"></span> \
           {@/if}\
           {@if (it.role ==6)}\
          ...
          \{@/if}\
          {@if it.rank >0}\
           <a href="http://js.artand.cn/about/collector"  class="btn cGrade grade${it.rank}"></a>\
          {@/if}\
         </h3>\
                                </div>\
                            </li>\
      {@/each}\
        ...
        function() {
            ...
            hideCls = 'hide',
            drop,
            append = $('<div class="sign_spread_out hide"></div>').on('select', 'li',
            function(ev) {
                var elem = $(this),
            ...
            var hanlde = function(list) {
                if (list && list.length) {
                    append.html(drop = $(juicer(TMPL, list))) $this.trigger('open');
            ...
            search = function(val) {
                ...
                if (!cache[key]) {
                    clearTimeout(itv) itv = setTimeout(function() {
                        xhr && xhr.abort && xhr.abort();
                        xhr = $.get('/proxy/search', {
                            term: val
                        },
                        function(result) {
                            if (result.code == 1000) {
            ...
            }).on('click', false).on('open',
            function(ev) {
                $(document.body).on('click.autocomplete',
                function(ev) {
...
(function() { (function(factory) {
    ...
    at_config = {
        ...
        tpl: '<li class="clearfix" data-value="@${uname}(${domain})">\
          <div class="head clearfix"><a href="http://js.artand.cn/${url}" class="btn imgbtn"> <img src="${head}" /></a></div>\
          <div class="writing">\
            ...
            remote_filter: function(query, render_view) {
                ...
                if (!self.data('active') && thisVal.length >= 1) {
                    ...
                    if (typeof itemsMentions == "object") {
                        ...
                        function(data) {
                            cachequeryMentions[thisVal] = data render_view(data);
                        },
                        'json');
                    }
    ...
    'json') window.at_config = at_config;
})($, window);;
$(function($) {
    var emotions = Spine.Controller.sub({
        ...
        init: function() {
            ...
            function() {
                if (!_this.items) {
                    ...
                    function(result) {
                        if (result.code == 1000) {
                            ...
                            var html = juicer('{@each _ as it}<li><img src="../../static.artandus.com/images/1x/${it.icon}.png"/*tpa=http://static.artandus.com/images/1x/${it.icon}.png*/ data-value="[${it.value}]" alt="${it.value}" title="${it.value}"/></li>{@/each}', _this.items);
                ...
                return false;
            }) _this.container.on('click.emotions',
            function() {
                _this.$el.hide();
            })
        },
        fnSelectFace: function(ev) {
            ...
            this.setCursor(ctrl, pos); ! ev.ctrlKey && this.$el.hide();
            return false;
        },
        ...
        if (options.async) {
            ...
            return {
                send: function(_, completeCallback) {
                    ...
                    counter += 1;
                    iframe = $('<iframe src="javascript:false;" name="iframe-transport-' + counter + '"></iframe>').bind('load',
                    function() {
    ...
    $.widget('blueimp.fileupload', {
        ...
        _getDroppedFiles: function(dataTransfer) {
            ...
            if (items && items.length && (items[0].webkitGetAsEntry || items[0].getAsEntry)) {
                return this._handleFileTreeEntries($.map(items,
                function(item) {
                    var entry;
        ...
        var thisDia = $(juicer(document.getElementById('message_tmpl').innerHTML, opts)).appendTo(document.body).easyModal({
            ...
            onOpen: function() {
                ...
                function(ev) {
                    ...
                    var atwho = text.atwho(at_config).data('atwho') $(window).on('resize.atwho',
                    function() {
                ...
                function() {
                    emotions.$el.hide();
                }) var file = $('input[type=file]', $this).bind('fileuploadadd',
                function(e, data) {
                    var $this = $(this),
                    ...
                    function(file) {
                        if (file.name && !acceptFileTypes.test(file.name)) {
                            $.notify('只能上传JPG，JPEG，PNG格式', 'error') return;
                        }
                ...
                function(e, data) {
                    ...
                    if (result.code == 1000) {
                        ...
                        span.find('http://js.artand.cn/concat/em.name').html(html).closest('.phiz').removeClass('on');
                    }
                }).prop('disabled', true);
                ...
                $this.on('click', '.pic_delete',
                function(ev) {
                    var $btn = $(this),
                    span = $btn.closest('.img_name_span'),
                    box = span.closest('.phiz');
                    box.addClass('on');
                    span.find('http://js.artand.cn/concat/em.name').empty();
                    return false;
                }).on('click', '.submit',
                function(ev) {
                    $(this).closest('form').trigger('submit');
                ...
                function(ev) {
                    ...
                    text = $('textarea', $this),
                    pics = $('http://js.artand.cn/concat/input.pics', $this);
                    if (!input.length) {
                    ...
                    $.post('/proxy/doMessage', $this.serialize(),
                    function(result) {
                        if (result.code == 1000) {
                            opts.onSuccess();
            ...
            function(result) {
                if (result.code != 1000) {
                    ...
                    case 0:
                        ...
                        break;
                    case 1:
                        var html = '{@if mutual}<span class="hover"><i>-</i>取消关注</span><span class="label">互相关注</span>{@else}<i>-</i>取消{@/if}'$this.attr('class', 'btn attract do-follow').html(juicer(html, result));
                        break;
                    }
    ...
    a.fn.fluidbox = function(b) {
        ...
        k = function(b) {
            if (a(this).hasClass("fluidbox")) {
                ...
                }).one("webkitTransitionEnd transitionend oTransitionEnd MSTransitionEnd transitionEnd",
                function() {
                    e.css({
                        opacity: 1
                    }),
        ...
        c.closeTrigger && a.each(c.closeTrigger,
        function(b) {
            var d = c.closeTrigger[b];
            "window" != d.selector ? "document" == d.selector ? d.keyCode ? a(document).on(d.event,
            function(a) {
    ...
    Events = {
        bind: function(ev, callback) {
            ...
            for (_i = 0, _len = evs.length; _i < _len; _i++) {
                name = evs[_i]; (_base = this._callbacks)[name] || (_base[name] = []);
        ...
        function Controller(options) {
            ...
            while (parent_prototype = context.constructor.__super__) {
                if (parent_prototype.events) {
                    this.events = $.extend({},
                    parent_prototype.events, this.events);
                }
                if (parent_prototype.elements) {
                    this.elements = $.extend({},
                    parent_prototype.elements, this.elements);
                }
    ...
    function(ev) {
        ...
        card: function(node) {
            ...
            function(ev, uid) {
                delete($.hash[uid]);
                return false;
            }) $(node).on('mouseenter mouseleave', '[data-card-id]',
            function(ev) {
...
(function($) {
    ...
    $.fn.autosize = function(options) {
        ...
        return this.each(function() {
            ...
            $ta.on('autosize.resizeIncludeStyle',
            function() {
                mirrored = null;
                adjust();
            });
            $ta.on('autosize.destroy',
            function() {
                mirrored = null;
                clearTimeout(timeout);
...
$(function($) {
                ...
                if (result.code == 1000) {
                    ...
                    if ($CONF['last_id'] = result.last_id) {
                        if (times >= 5) {
                            ...
                            $window.on('http://js.artand.cn/concat/scroll.lazy', handleLoad);
                        }
                    } else {
                        more.parent().remove();
                    }
    ...
    autoLoad();
    $window.on('layzLoad', layzLoad) var eventHome = {
        setup: function() {
            var main = $('.main-body'),
            isLock;
            ...
            if (!isMobile) {
                ...
                this.surface.on('http://js.artand.cn/concat/inview.home',
                function(ev, visible) {
                    $window.on('http://js.artand.cn/concat/scroll.home',
                    function() {
                        if (visible) {
                            didScroll = true;
                        }
                    })
                }) $window.on('http://js.artand.cn/concat/gotop.home', $.proxy(function() {
                    this.surface.trigger('inview');
                },
                this));
            }
            this.surface.on('http://js.artand.cn/concat/click.home', '.downwards',
            function() {
            ...
            var publish_new = $('#publish_new') $window.on('publish.success', $.proxy(function(ev, html) {
                $(html).insertAfter(publish_new).find('img[data-src]').attr('src',
                function() {
                    return $(this).attr('data-src');
                }).removeAttr('data-src');
            },
            this)).on('http://js.artand.cn/concat/loadMore.home',
            function(ev, html) {
                var elem = $(html);
    ...
    var eventCollections = {
        setup: function() {
            ...
            $CONF['is_owner'] && this.container.sortable({
                items: 'http://js.artand.cn/concat/li.item',
            ...
            $window.on('loadMore.collections',
            function(ev, html) {
                var elem = $(html);
                container.append(elem);
                $window.trigger('layzLoad').trigger('scroll');
        ...
        setup: function() {
            if (!$CONF['isMobile']) {
                ...
                function() {
                    ...
                    var $this = $(this) if ($this.outerWidth(true) >= 200) {
                        if (!$this.parent().is('a')) {
                            var a = $('<a></a>').attr('href', $this.attr('src'));
            ...
            function(result) {
                if (result.code == 1000) {
                    ...
                    if (result.page_id == 'home' && ($CONF['about_tip'] = result.about_tip)) {
                        $body.trigger('http://js.artand.cn/concat/accept.tip');
                    } else {
                        $('.um_tip_tab').remove();
                    }
...
$(function($) {
    ...
    var getDia = function() {
        if (dialog) {
            ...
            return dialog = $(juicer(document.getElementById('move_collection_tmpl').innerHTML, $CONF['collections'])).on('close',
            function(ev) {
                var $this = $(this),
                li = $this.data('li');
            ...
            }).on('click', '.create-collection-btn',
            function(ev) {
                var $this = $(this),
    ...
    $body.on('click', '.collection li a.move-collection',
    function(ev) {
        var $this = $(this),
        dialog = getDia(),
        offset = $this.offset(),
        ...
        offset.left -= (dialog.outerWidth() - $this.outerWidth()) dialog.css(offset).attr('row_id', row_id).data('li', li);
    ...
    }).on('click', '.collection li a.reset_show',
    function(ev) {
        var $this = $(this),
        row_id = $this.attr('data-row-id');
...
$(function($) {
        ...
        if (!elem.length) {
            var handleBlur = function() {
                ...
                if (val.length) {
                    ...
                    span.html($L('“点击这里，编写个人签名”')).attr('class',
                    function() {
    ...
    function(ev) {
        ...
        return false;
    }).on('click', '#collection_create_btn',
    function(ev) {
        $.createCollection();
        return false;
    }).on('mouseenter mouseleave', '#container li',
    function(ev) {
        var $this = $(this),
        isLock = $this.hasClass('lock');
        switch (ev.type) {
    ...
    function(ev) {
        ...
        'json');
        return false;
    }) $body.on('http://js.artand.cn/concat/accept.tip',
    function() {
        if ($CONF['about_tip']) {
    ...
    $.extend({
        ...
        editeCover: function(opts) {
            ...
            opts || {}),
            $body = $(document.body),
            thisDia = $(juicer(document.getElementById('collection_tmpl_edite').innerHTML, opts)).on('open',
            function() {
            ...
            function() {
                $body.off('keydown.create').attr('style',
                function() {
            ...
            $('form', thisDia).validate({
                ...
                submitHandler: function(form) {
                    $.post('/u/doCollection', $(form).serialize(),
                    function(result) {
                        if (result.code == 1000) {
                            $.notify($L('操作成功'), {
                                callback: function() {
        ...
        changeCover: function(collection_id) {
            ...
            var handleCover = function(TMPL) {
                ...
                function() {
                    ...
                    }).on('http://js.artand.cn/concat/keydown.esc',
                    function(ev) {
                        if (ev.keyCode == 27) {
                            thisDia.trigger('close');
                        }
                ...
                function() {
                    thisDia.remove();
                    $body.attr('style', $style).off('http://js.artand.cn/concat/keydown.esc') return false;
                }).on('click', '.reset',
                function() {
                    thisDia.trigger('close');
                    return false;
                ...
                function(ev) {
                    ...
                    collection_id = $cover.attr('data-collection-id') $data = $.extend({
                        fid: pid,
                        row_id: row_id,
                        collection_id: collection_id
                    },
                    position);
                    $.post('/upload/crop_collection', $data,
                    function(result) {
                        if (result.code == 1000) {
...
$(function($) {
    ...
    $(document.body).on('click', '.view-resume',
    function() {
        if (!RESUME) {
            $.get('/u/resume', {
...
$(function($) {
    ...
    var createDialog = function(opts) {
        if (!thisDia) {
            thisDia = $(juicer(document.getElementById('collection_tmpl_create').innerHTML, opts || {})).on('open',
            function() {
                $(document.body).on('keydown.create',
                function(ev) {
                    if (ev.keyCode == 27) {
                        thisDia.trigger('close')
                    }
            ...
            function() {
                ...
                $('html').removeClass('dialog') return false;
            }).on('click', '.reset',
            function() {
                thisDia.trigger('close');
                return false;
                    ...
                    function(result) {
                        if (result.code == 1000) {
                            ...
                            if (result.data && result.data.collection_id) {
                                location.href = "http://js.artand.cn/works/collection/" + result.data.collection_id;
                            } else {
                                location.reload();
                            }
...
$(function($) {
    ...
    thisDia,
    $style;
    $body.on('click', '.like-privacy',
    function(ev) {
        if (!thisDia) {
            thisDia = $(document.getElementById('like_privacy_tmpl').innerHTML).on('open',
            function() {
                $style = $body.attr('style') || '';
                $body.css({
                    overflow: 'hidden',
                ...
                $body.on('http://js.artand.cn/concat/keydown.esc',
                function(ev) {
                    if (ev.keyCode == 27) {
                        thisDia.trigger('close');
                    }
            ...
            function() {
                $body.off('http://js.artand.cn/concat/keydown.esc').attr('style', $style);
                thisDia.detach();
            }).on('click', '.reset',
            function() {
                thisDia.trigger('close');
...
$(function($) {
    $(document.body).on('click', '#create-article-btn',
    function() {
        new $.umeditor({
            $body: $('body'),
            $html: $('html'),
...
$(function($) {
    var $body = $(document.body);
    if ($CONF['is_owner']) {
        $('.title_page').on('mouseenter mouseleave',
        function(ev) {
            $('.cover-change', this).toggleClass('hide', ev.type == 'mouseleave');
        })
    }
    $body.on('click', '.cover-change',
    function() {
        var $this = $(this),
        collection_id = $this.attr('data-collection-id');
        $.changeCover(collection_id);
        return false;
    }).on('click', '.collection-edite',
    function() {
        var $this = $(this),
        param = $this.attr('data-param');
        $.editeCover($.parseJSON(param || false));
        return false;
    }).on('click', '.collection-delete',
    function() {
        $.delCollection();
        return false;
    }) $.extend({
        delCollection: function() {
            var thisDia = $(document.getElementById('collection_del').innerHTML).appendTo(document.body).on('click', '.reset',
            function() {
                thisDia.trigger('closeModal');
                return false;
            }).on('click', '.submit',
            function() {
                ...
                onOpen: function() {
                    var validate = $(this).validate({
                        ...
                        focusInvalid: true,
                        submitHandler: function(form) {
                            $.post('/u/deleteCollection', $(form).serialize(),
                            function(result) {
                                if (result.code == 1000) {
                                    $.notify('删除成功', {
                                        callback: function() {
...
$(function() {
    if ($CONF['is_owner']) {
        $(document.body).on('click', 'li .show-profile',
        function(ev) {
...
$(function() {
    $(window).on('sns_bind_callback',
    function(ev, type, isBind) {
    ...
    $('.verify_collect,.Explanation').on('mouseenter mouseleave',
    function(ev) {
        var $this = $(ev.currentTarget);
        switch (ev.type) {
        case 'mouseenter':
...
$.extend({
    reward: function(params) {
        ...
        pushstream,
        surplus,
        $el = $(ju).appendTo(document.body);
        $.getScript('../../static.artandus.com/script/plugin/jquery.qrcode.min.js'
        /*tpa=http://static.artandus.com/script/plugin/jquery.qrcode.min.js*/
        );
     ...
     <div class="modal-body">\
         <div class="headWarp">\
             <div class="head"></div>\
             <h3>打赏成功</h3>\
             <a class="btn t_links_blue" href="../../artand.cn/login-callback=-setting-wallet.htm"/*tpa=http://artand.cn/setting/wallet*/>查看打赏记录</a>\
         </div>\
     </div>';
        var showsurplus = function() {
            $.post('/payment/surplus',
            function(result) {
                if (result.code == 1000) {
            ...
            onOpen: function() {
                ...
                function(ev) {
                    ...
                    case 'wxpay':
                        ...
                        var serialize = $el.serialize();
                        $.post('/payment/wxpay/reward', serialize,
                        function(result) {
                            $button.removeClass('lock').text('立即支付');
                            if (result.code == 1000) {
                                var sn = result.sn;
...
$(function($) {
    ...
    $(document.body).on('click', '.reward-more',
    function(ev) {
        var $this = $(this),
        id = $this.attr('data-id'),
        type = $this.attr('data-type') reward(id, type, $this.closest('div'));
        return false;
    })
});;
$(function($) {
            ...
            function(result) {
                if (result.code != 1000) {
                    ...
                    if (!like) {
                        ...
                        $(juicer('<a href="http://js.artand.cn/${domain}" title="${uname}" class="btn head_p_btn head_${uid}"><img src="${head}" alter="${uname}"></a>', $.extend($CONF, result))).prependTo(personBox);
                    }
    ...
    function(ev) {
        ...
        $.post('/proxy/doComment', $this.serialize(),
        function(result) {
            if (result.code == 1000) {
    ...
    function() {
        ...
        return false;
    }).on('mouseenter mouseleave', '#comment_list li',
    function(ev) {
    ...
    }).on('focusin', '#comment_form textarea',
    function() {
        checkLogin();
        lazyForm(false);
    }).on('click', '.phiz_btn',
    ...
    var lazyForm = function(trigger_click) {
        ...
        if ($CONF['is_login'] && !emotions_btn.data('emotions')) {
            ...
            emotions_btn.after(emotions.$el) textarea.closest('form').on('submit',
            function() {
                emotions.$el.hide();
            }) if (!$CONF['isMobile']) {
    ...
    if ($.reward) {
        $(document.body).on('click', '.btn_reward',
        function() {
            $.reward({
                type: 13
            });
            ...
            dialog = $(juicer(document.getElementById('report_tmpl').innerHTML, params)).appendTo(document.body).easyModal({
                ...
                onOpen: function() {
                    ...
                    function() {
                        ...
                        $.post('/proxy/doReport', $(this).serialize(),
                        function(result) {
                            if (result.code == 1000) {
                                $.notify($L('举报成功'));
                                $this.trigger('closeModal');
