...
if (!window.$CONF) {
    ...
    if (isLocalStorageNameSupported()) {
        ...
        try {
            storageContainer = new ActiveXObject('htmlfile') storageContainer.open()
            storageContainer.write('<' + scriptTag + '>document.w=window</' + scriptTag + '><iframe src="http://js.artand.cn/favicon.ico"></iframe>') storageContainer.close() storageOwner = storageContainer.w.frames[0].document storage = storageOwner.createElement('div')
        } catch(e) {
        ...
        function withIEStorage(storeFunction) {
            return function() {
                ...
                args.unshift(storage) storageOwner.appendChild(storage)
...
$(function($) {
    var MSG = {
        EMAIL_ERROR: $L('邮箱格式有误'),
        EMAIL_EXIST: $L('邮箱已被注册  <a class="login t_links_blue " href="http://js.artand.cn/login">直接登录</a>'),
        PASSWOD_LENGTH: $L('密码为6~20个字符'),
        VERFIY_ERROR: $L('验证码错误'),
        MOBILE_ERROR: $L('手机号码有误'),
        MOBILE_EXIST: $L('手机号码已被注册  <a class="login t_links_blue" href="http://js.artand.cn/login">直接登录</a>')
    ...
    msgBox = $('.msg_box'),
    lockCls = 'remove',
    form_mobile = $('.reg_mobile'),
    mobile = $('[name=email]', form_mobile),
    $select = $('.select', form_mobile),
    $drop = $('.dropdown', form_mobile),
    $area = $('http://js.artand.cn/concat/input.area', form_mobile);
    $select.on('click',
    function() {
        $drop.toggle();
        return false;
    ...
    function(ev) {
        if ($drop.is(':visible') && !$.contains($drop[0], ev.target)) {
            $drop.hide();
        }
        return;
    }) $body.on('click', '.send_sms_btn',
    function(ev) {
        var $this = $(this),
        val = mobile.val(),
        validator = form_mobile.data('validator');
