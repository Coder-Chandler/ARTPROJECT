...
$(function($) {
    ...
    uname = $('input[name=email]', form),
    password = $('input:password', form),
    verify = $('img.verify', form),
    row = verify.closest('.text_forms'),
    validate = host = location.host;
    box.on('click', '.weixin_toggle',
    function() {
    ...
    function() {
        var obj = new WxLogin({
            ...
            appid: "wxd2ea93b2429b30bd",
            scope: "snsapi_login,snsapi_userinfo",
            redirect_uri: encodeURIComponent("http://" + location.host + "/login2/wechat/token?callback=" + encodeURIComponent($CONF['callback'] || "/")),
            state: $CONF['state'],
            style: "black"
        });
    });
    ...
    form.on('click', '.verify-refresh', $.refresh);
    function openwindow(url, name, iWidth, iHeight) {
        var url, name, iWidth, iHeight, iTop = (window.screen.availHeight - 30 - iHeight) / 2,
        iLeft = (window.screen.availWidth - 10 - iWidth) / 2;
        return window.open(url, name, 'height=' + iHeight + ',,innerHeight=' + iHeight + ',width=' + iWidth + ',innerWidth=' + iWidth + ',top=' + iTop + ',left=' + iLeft + ',toolbar=no,menubar=no,scrollbars=auto,resizeable=no,location=no,status=no');
    }
    $(document.body).on('click', '[data-sns]',
    function(ev) {
        var url = $(this).attr('data-sns');
        if ($CONF['isMobile'] || $CONF['isIpad']) {
            location.href = url;
        } else {
            var op = openwindow(url, null, 752, window.screen.height);
            op && op.focus();
        }
        $.success = function(result) {
            if (result.code == 1000) {
        ...
        return false;
    }) var domain_url = function(result) {
        if (result['domain']) {
            return '/' + result['domain'];
        } else {
            return '/uid/' + (100000 + parseInt(result['uid']));
        }
    }
    $.success = function(result) {
        $CONF['is_login'] = true;
