...
$(function() {
    ...
    var $message = $('#message'),
        ...
        messageReceived = function(result, id, channel) {
            var html,
                last_message = store.get('message'),
...
(function($) {
    ...
    var dialog = {
        login: function(form) {
            var verify = $('img.verify', form),
                ...
                validate = form.on('click', '.submit', function() {
                    ...
                    focusInvalid: true,
                    submitHandler: function() {
                        $.post('/proxy/doLogin', form.serialize(), function(result) {
                            if (result.code != 1000) {
                                if (result.verfiy) {
                                    row.show();
                                    refresh(verify);
        ...
        regist: function(form) {
            ...
            form_mobile.validate({
                ...
                submitHandler: function(el, ev) {
                    ...
                    form.addClass('lock'), validate = form.data('validator');
                    $.post('/proxy/doSignup', serialize, function(result) {
                        if (result.code == 1000) {
                            $.notify($L('注册成功'), {
                                cls: 'success',
                                callback: function() {
        ...
        show: function(opts) {
            ...
            }, opts || {})))), login = $('.forms_login', modal), regist = $('.forms_regist', modal);
...
$(function($) {
    ...
    var width = ul.width(),
        height = width,
        li = $('li img[data-src]', ul);
    var $el = $('a.do-follow').attr('src', function() {
        return '/login';
    });
    if ($CONF['isMobile']) {
        li.css({
